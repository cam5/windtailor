...
[`docs/TECH_DEBT.md`](docs/TECH_DEBT.md) is windtailor's classified tech-debt register: a scoring
rubric (impact × likelihood ÷ effort → P0–P3) followed by every known item, each pinned to a
`file:line` and paired with a remediation sketch.

The register's *mechanical* half — modules with no colocated test, marker comments, TypeScript
escape hatches, oversized files — is regenerated by a dependency-free scanner:

```sh
npm run debt            # human-readable
npm run debt -- --json  # machine-readable
```

Run it to check whether the counts in the register have drifted. It only reports what can be
counted; architectural debt (missing test seams, duplicated invariants, dependency pins) is
recorded by hand in the document itself.
